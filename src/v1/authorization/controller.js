...
const signUpTeacher = async (req, res) => {
    try {
        ...
        if (isAlready) {
            return res.send(new serviceResponse({ status: 400, errors: [{ message: "This email already exists" }] }))
        }
...
const loginAdmin = async (req, res) => {
    try {
        ...
        if (!admin) {
            return res.send(new serviceResponse({ status: 400, errors: [{ message: "No admin found corrosponding to this email address" }] }))
        }
        // Password validation
...
const forgotPassword = async (req, res) => {
    try {
        ...
        return res.send(new serviceResponse({ status: 200, data:admin, message: "Reset password otp has been dispatched to your registered email" }))
    } catch (error) {
        return res.status(200).send(new serviceResponse({ status: 500, errors: [{ message: `${error.message}` }] }))
    }
}
